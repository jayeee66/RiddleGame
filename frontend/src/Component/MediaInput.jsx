...
function MediaInput({ media, mediaType, onChange }) {
  const [tab, setTab] = useState(mediaType === 'video' ? 'video' : 'image');
  const [videoUrl, setVideoUrl] = useState(mediaType === 'video' ? (media || '') : '');
      ...
      {media ? (
        ...
        <div className="flex gap-2">
          <input
            ...
            onKeyDown={e => e.key === 'Enter' && videoUrl.trim() && onChange(videoUrl.trim(), 'video')}
          ...
          />
          <button
            onClick={() => videoUrl.trim() && onChange(videoUrl.trim(), 'video')}
