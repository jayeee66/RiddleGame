...
// ✅ Node 22+/23 用 import attributes（with），不是 assert
...
app.put('/admin/games', catchErrors(authed(async (req, res, email) => {
  ...
  if (!Array.isArray(games)) throw new InputError("Games must be an array");
...
const configPath = path.resolve(__dirname, '../../frontend/backend.config.json');
